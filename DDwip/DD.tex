...
\item \textbf{Account Manager}
    ...
    \begin{itemize}
        \item \textbf{getCreatedBattles(userId):} This method takes the user's ID from the session data, contacts the DBMS which will check if the 
        ...
        different types of data will be provided, also relative to the battle provided.
        If an error occurs while interacting with the DBMS, an error page will be provided.
        \item \textbf{enterBattle(groupId, battleId):} This method takes the group's ID and battle's ID and checks whether the group exists, if the 
    ...
    \begin{itemize}
        \item \textbf{getCreatedBadges(userId):} This method takes the user's ID from the session data, contacts the DBMS which will check if the 
...
\hspace*{-2cm}\begin{tabular}{|g|c|c|}
    ...
    \vspace{\baselineskip}}                     & \makecell[l]{
        \textit{2h}     \\\textit{16h}          \\\textit{3h}           \\\textit{3h}             \\\textit{2h} \vspace{\baselineskip}}\\
    \hline
    \textbf{Luca Tosetti}                       & \makecell[l]{                               
        \\Introduction  \\Architectural Design  \\User Interface Design\\Requirement Traceability\\Implementation, Integration and Test plan\\
    \vspace{\baselineskip}}                     & \makecell[l]{
        \textit{3h}     \\\textit{13h}           \\\textit{3h}          \\\textit{3h}             \\\textit{4h}}\\
    \hline
    \textbf{Francesco Riccardi}                 & \makecell[l]{                               
        \\Introduction\\Architectural Design    \\User Interface Design\\Requirement Traceability\\Implementation, Integration and Test plan\\
    \vspace{\baselineskip}}                     & \makecell[l]{
        \textit{1h}   \\\textit{7h}             \\\textit{16h}         \\\textit{1h}             \\\textit{1h} \vspace{\baselineskip}}\\
    \hline
\end{tabular}
